...
if (marks >= 90) {
    ...
    console.log(`Marks are ${marks} and the grade is Fail)`)
}
//This is using the conditional statements

let mark = prompt("Enter the marks :");
let grade = (mark >= 90 ) ? `Marks are ${mark} and the grade is A` : (mark >= 70) ? `Marks are ${mark} and the grade is B` : (mark >= 50) ? `Marks are ${mark} and the grade is C` : `Marks are ${mark} and the grade is Fail`;
console.log(grade);
